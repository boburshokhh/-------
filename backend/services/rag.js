...
function estimateThemeCount(indexedChunks, fullText) {
    if (indexedChunks && indexedChunks.length > 0) {
        let factsCount = 0;
        for (const chunk of indexedChunks) {
            if (Array.isArray(chunk.summary)) factsCount += chunk.summary.length;
        }
    ...
    for (let attempt = 1; attempt <= 3; attempt++) {
        try {
            ...
            if (themes && themes.length > 0) {
                ...
                console.log(`[RAG] extractThemes: ${themes.length} тем из ${indexedChunks ? indexedChunks.length : 0} чанков`);
                return themes;
            }
...
function buildEvidencePackets(chunks, _intent, opts = {}) {
    ...
    return chunks.map(chunk => {
        const facts = Array.isArray(chunk.summary) && chunk.summary.length > 0 ? chunk.summary : [];
        const text = chunk.text.length > maxTextChars ? chunk.text.slice(0, maxTextChars) + '…' : chunk.text;
        return { chunk_id: chunk.id, facts, text, page: chunk.page ?? null, section: chunk.section ?? null };
    });
}
