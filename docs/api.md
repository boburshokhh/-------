...
Требует авторизацию и роль `admin`.
- **Ответ (200 OK)**:
  ```json
  { "success": true }
  ```
...
### 7. Сохранение результатов прохождения
**POST** `/api/results`
...
### 8. Получение списка результатов для конкретного теста
...
### 9. Получение деталей конкретного результата
...
### 10. Просмотр backend-логов (debug)
...
### 11. Скрытые runtime-настройки (admin)
...
### 12. AI Admin: Mode Profiles (custom modes)
