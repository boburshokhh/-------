...
### Сообщение в логах: `503` / `UNAVAILABLE` / «high demand» (Gemini)
Это **временная перегрузка модели на стороне Google**, не ошибка вашего кода. Сервис делает до **5** повторов с паузами **8 → 16 → 32 → …** секунд (до лимита). Подождите и повторите загрузку; при частых сбоях переключите модель в UI на **gemini-2.5-flash** (часто стабильнее для free tier).
