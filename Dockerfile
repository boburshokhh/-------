...
HEALTHCHECK --interval=30s --timeout=5s --start-period=45s --retries=5 \
    CMD node -e "require('http').get('http://127.0.0.1:3002/api/health',(r)=>{const ok=r.statusCode===200;r.resume();r.on('end',()=>process.exit(ok?0:1));r.on('error',()=>process.exit(1));}).on('error',()=>process.exit(1));"
