...
Если в логах `password authentication failed for user "…"`: в Docker Compose приложение подключается как **`ai_testgen`**, а не как `PGUSER` из `.env` (имя в `.env` вроде `bobur` для локальной разработки не используется в контейнере `app`). Пароль — **`PGPASSWORD` из `.env`**, он должен совпадать с тем, что был при **первом** создании тома Postgres; при смене только пароля без пересоздания тома — сбой входа. Исправление: вернуть старый пароль или `docker compose down -v` (удалит данные БД) и поднять заново. Если том изначально создавался с другим суперпользователем (не `ai_testgen`), сначала выровняйте роли вручную в Postgres или пересоздайте том.
