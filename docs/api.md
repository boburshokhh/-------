...
Возвращает список всех сгенерированных тестов (по сохранённому ручному порядку `sort_order`).
- **Ответ (200 OK)**:
  ```json
  {
    "tests": [
...
### 5. Экспорт теста в JSON
**GET** `/api/tests/:id/export`

Возвращает сохранённый тест в формате `ai_test_export.v1` для скачивания на клиенте. Вопросы берутся из исходного `questions_json`, поэтому в экспорт попадают подсказки, объяснения и правильные ответы.
- **Ответ (200 OK)**:
  ```json
  {
    "schema_version": "ai_test_export.v1",
    "exported_at": "2026-04-26T05:47:00.000Z",
    "test": {
      "id": 1,
      "title": "Тест по документу: История",
      "totalQuestions": 1,
      ...
      "document": {
        ...
        "extractionQuality": 0.94,
        "lowTextQuality": false,
        "parseDiagnostics": { "parseMethod": "pdf-parse" }
      }
    },
    "questions": [
      {
        ...
        "correct_answer": 2,
        "correctIndex": 2,
        "correctAnswerText": "1200",
        "hint": "Обратите внимание на дату в описании события.",
        "explanation": "Событие X произошло в 1200 году согласно летописям.",
...
### 7. Изменение позиции теста в библиотеке
**PATCH** `/api/tests/:id/position`

Меняет позицию теста в ручном порядке (на один шаг вверх/вниз).
- **Headers**: `Content-Type: application/json`
- **Body**:
  ```json
  { "direction": "up" }
  ```
  Допустимые значения `direction`: `up` или `down`.
- **Ответ (200 OK)**:
  ```json
  { "success": true, "moved": true }
  ```
  Если тест уже первый/последний, вернётся `moved: false`.
- **400**: некорректный `direction`.
- **404**: тест не найден.

---

### 8. Сохранение результатов прохождения
**POST** `/api/results`
...
### 9. Получение списка результатов для конкретного теста
...
### 10. Получение деталей конкретного результата
...
### 11. Просмотр backend-логов (debug)
...
### 12. Скрытые runtime-настройки (admin)
...
### 13. AI Admin: Mode Profiles (custom modes)
